...
if (hasIdParam()) {
    ...
    if (isset($editedTask->task) && isset($editedTask->task->id)) {
        ...
        if ($editedTask->status == 'CLOSED' || $editedTask->status == 'CANCELLED')
            $disabled = true;
        if ($editedTask->status == 'PICKED UP' || $editedTask->status == 'DELIVERED' || $editedTask->payment > 0) {
            $field_disabled = true;
        }
        $isUpdate = true;
...
// print_r($_SESSION['int_acl']['acl_rules'][$taskForm]);
    ...
    <div class="row">
        ...
        <article class="col-sm-12 col-md-12 col-lg-12">
            ...
            <div class="jarviswidget" data-widget-colorbutton="false" data-widget-editbutton="false">
                <header role="heading">
                    <h2 style="width:auto">Task Form
                        <?= $isUpdate ? 'ID: ' . $editedTask->id : '' ?>
                    </h2>
                    <?php
                    // $help_form = 'warranty';
                    ...
                    <?php if ($isUpdate) { ?>
                        <div class="jarviswidget-ctrls" id="task-form-control" role="menu">
                            <a href="./#ajax/task.php" class="jarviswidget-toggle-btn btn-primary have-text pl-5 pr-5"><i
                                    class="fa fa-plus"></i> Create new task</a>
                        </div>
                    <?php } ?>
                </header>
                <div>
                    ...
                    <form class="smart-form" id="task_form" method="post">
                        ...
                        <?php if ($isUpdate) { ?>
                            <input type="hidden" id="assign" data-name="<?= $editedTask->assign_name ?>"
                                data-value="<?= $editedTask->assign_id ?>">
                            <input type="hidden" id="customer" data-name="<?= $editedTask->cus_name ?>"
                                data-value="<?= $editedTask->customer_id ?>">
                            <input type="hidden" id="createDate" data-name="<?= $editedTask->createDate ?>"
                                data-value="<?= $editedTask->createDate ?>">

                            <input type="hidden" id="order-edit" data-value="<?= $editedTask->assign_order ?>"
                                data-name="<?= $editedTask->order_title ?>">
                            <input type="hidden" id="driver-edit" data-value="<?= $editedTask->assign_id ?>"
                                data-name="<?= $editedTask->assign_name ?>">
                            <input type="hidden" id="depot_id" data-value="<?= $editedTask->depot_id ?>"
                                data-name="<?= $editedTask->depot_id ?>">
                            <input type="hidden" id="container_type_id" data-value="<?= $editedTask->container_type_id ?>"
                                data-name="<?= $editedTask->container_type_id ?>">
                            <input type="hidden" id="mile" data-value="<?= $editedTask->mile ?>"
                                data-name="<?= $editedTask->mile ?>">
                            <input type="hidden" id="qty" data-value="<?= $editedTask->qty ?>"
                                data-name="<?= $editedTask->qty ?>">

                        <?php } ?>
                        <input type="hidden" name="createDate"
                            value="<?= ($isUpdate ? $editedTask->createDate : '') ?>">
                        <fieldset>
                            ...
                            <div class="row order-class">
                                <?php
                                if (hasPermission($taskForm, 'assign_order', 'show')) { ?>
                                    <section class="col col-6">
                                        <label class="input">Order</label>
                                        <select class="field-disabled" name="assign_order" id="assign_order"
                                            style="width:100%;"></select>
                                    </section>
                                <?php } ?>
                                <?php
                                if (hasPermission($taskForm, 'product_sku', 'show')) { ?>
                                    <section class="col col-6">
                                        <label class="label">Product(*)</label>
                                        <label class="input">
                                            <select id="product_sku" class="valid h32 field-disabled" style="width:100%"
                                                disabled=""></select>
                                        </label>
                                    </section>
                                <?php } ?>
                            ...
                            <div class="row order-class">
                                <?php
                                if (hasPermission($taskForm, 'assign_driver_id', 'show')) { ?>
                                    <section class="col col-6 driver-rate">
                                        <label class="input">Assign driver<span
                                                class="driver-rate-total color-alert">(Delivery:
                                                <?= $total; ?>)
                                            </span>
                                            <span class="<?= $clss_hidden; ?>" style="float: right">
                                                <span class="btn btn-sm btn-primary" id="btn-send-email">Send email to
                                                    driver</span></span>
                                        </label>
                                        <select name="assign_driver_id" id="assign_driver_id" style="width:100%"
                                ...
                                SmartForm::print_field('deliverydate', SmartForm::FORM_FIELD_INPUT, array(
                                    ...
                                    'type' => 'date',
                                    'class' => 'datepicker',
                                    'id'=>'delivery-date-task',
                                    'value' => $delivery_date
                                ), 4, false, hasPermission($taskForm, '', 'show'));

                                SmartForm::print_field('deliverytime', SmartForm::FORM_FIELD_INPUT, array(
                                    'label' => 'Delivery Time',
                                    'type' => 'time',
                                    'class' => 'timepicker',
                                    'id'=>'delivery-time-task',
                                    'value' => $delivery_time
                                ), 2, false, hasPermission($taskForm, 'deliverydate', 'show'));
                                ?>
                            </div>
                            ...
                            <div class="row" style="display: none">
                                ...
                                if (hasPermission($taskForm, 'assign_id', 'show')) {
                                    ...
                                    <section class="col col-6" id="is_show_assign_id">
                                        ...
                                        <select name="assign_id" id="assign_id" style="width:100%" class="form-control"
                                            <?= hasPermission($taskForm, 'assign_id', $task_current_form) ? '' : 'disabled="disabled"' ?>></select>
                                    </section>
                                <?php } ?>
                                <?php
                                if ($isUpdate && hasPermission($taskForm, 'customer_id', 'show')) { ?>
                                    <section class="col col-6">
                                        <label class="input">Customer <span class="link_to" data-view="link_to"
                                                data-form="#task_form" data-control="customer_id" data-name="contact-form"
                                                data-param="id"></span></label>
                                        <select name="customer_id" id="customer_id" style="width:100%" class="form-control"
                                            <?= hasPermission($taskForm, 'customer_id', $task_current_form) ? '' : 'disabled="disabled"' ?>></select>
                                    </section>
                                <?php } ?>
                            </div>
                            <div class="row">
                                ...
                                */
                                SmartForm::print_field('status', SmartForm::FORM_FIELD_SELECT, array(
                                    'label' => 'Status',
                                    'data' => array(
                                        array('label' => 'Scheduling', 'value' => 'NEEDS TO BE SCHEDULED'),
                                        array('label' => 'Scheduled', 'value' => 'SCHEDULED FOR DELIVERY'),
                                        array('label' => 'Out for delivery', 'value' => 'PICKED UP'),
                                        array('label' => 'Delivered', 'value' => 'DELIVERED'),
                                        array('label' => 'Billing and close out', 'value' => 'CLOSED'),
                                        array('label' => 'Cancelled', 'value' => 'CANCELLED'),
                                        array('label' => 'Problems', 'value' => 'PROBLEMS')
                                    ),
                            ...
                            <div class="row">
                                <div class="col col-6 m-b10 m-t10">
                                    <div class="form-group">
                                        ...
                                        <div class="col col-12 b-gray51" style="min-height: 130px">
                                            <div class="col col-12" id="upload-attachement">
                                                <?= $file_upload; ?>
                                            </div>
                                            ...
                                            <div class="col-12 file_upload text-center m-t10">
                                                <label for="upload">
                                                    <a class="btn btn-primary text-light " role="button"
                                                        aria-disabled="false">Click here to upload file</a>
                                                </label>
                                                <input type="file" name="upload_file[]" accept=".pdf,image/*"
                                                    id="upload" style="visibility: hidden; position: absolute;"
                                                    multiple />
                                            </div>
                                ...
                                <div class="col col-6 m-b10 m-t10">
                                    <div class="form-group">
                                        ...
                                        <div class="col col-12 b-gray51" style="min-height: 130px">
                                            <div class="col col-12" id="unload-attachement">
                                                <?= $file_unload ?>
                                            </div>
                                            ...
                                            <div class="col-12 file_upload m-t10 text-center">
                                                ...
                                                <input type="file" name="unload_file[]" accept=".pdf,image/*"
                                                    id="unload" style="visibility: hidden; position: absolute;"
                                                    multiple />
                                            </div>
                            ...
                            if ($need_pay_driver == 1) { ?>
                                <fieldset class="fs fs-property m-t10 m-b25" id="tb-info-driver">
                                    ...
                                    ?>
                                    <div class="modal fade animated fadeInDown" style="display:none; margin:auto;"
                                        id="modal-pay-to-driver">
                                ...
                                <fieldset class="fs fs-property m-t10 m-b25" id="session-payment-driver"
                                    ...
                                    <table id="tb-payment-driver" class="table table-tripped table-bordered"
...
<script type="text/javascript">
    var product_sku_eidt = []
    var is_disabled = '<?php echo $disabled; ?>';
    var field_disabled = '<?php echo $field_disabled; ?>';
</script>
<?php if ($isUpdate) { ?>
    <script type="text/javascript">     product_sku_eidt = '<?php echo $editedTask->product_sku; ?>';
    </script>
<?php } ?>
<script src="<?= ASSETS_URL ?>/js/script/common_f.js"></script>
...
<?php if ($isUpdate) { ?>
    <script>     new ControlPage('#task-form-control');
    </script>
