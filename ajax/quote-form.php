        ...
        <article class="col-sm-12 col-md-12 col-lg-12">
            ...
            <div class="jarviswidget" data-widget-colorbutton="false" data-widget-editbutton="false">
                <header id="quote-form-load">
                    <h2 style="width:auto">Quote Form
                        <?= hasIdParam() && $order_current_permission == true ? 'edit: <span id="heading_title_id"></span>' : '' ?>
                    </h2>
                    <?php
                    //$help_form = 'order';
                ...
                <div class="jarviswidget-body" style="max-width:100%">
                    ...
                    if ($canAddContact && basename($_SERVER['PHP_SELF']) == 'quote-form.php') { ?>
                        ...
                        <div class="modal animated fadeInDown" style="display:none; margin:auto; max-height:600px;"
                    ...
                    <form class="smart-form" id="order_form" method="POST">
                        ...
                        <fieldset>
                            <div class="row">
                                ...
                                <?php
                                if (hasPermission($order_form, 'order_title', 'show')) { ?>
                                    <section class="col col-6">
                                        <label class="input">Order Title:</label>
                                        <input type="text" class="form-control is_disabled" name="order_title"
                                            <?= hasPermission($order_form, 'order_title', $operation_order) ? '' : ' readonly' ?>>
                                    </section>
                                <?php } ?>
                                <?php
                                if (hasPermission($order_form, 'order_zipcode', 'show')) { ?>
                                    <section class="col col-xs-<?= $canAddContact ? 5 : 6 ?>">
                                        <label class="input">Zipcode<span class="c-orange">(*, first selected):
                                            </span></label>
                                        <input name="order_zipcode" class="form-control select2 is_disabled"
                                            id="order_zipcode" style="width:100%" <?= hasPermission($order_form, 'order_zipcode', $operation_order) ? '' : ' disabled' ?>></select>
                                        <p></p>
                                    </section>
                                <?php } ?>
                                <?php
                                if (hasPermission($order_form, 'salesperson', 'show')) { ?>
                                    <section class="col col-6">
                                        <label class="input">Salesperson: <span class="link_to" data-view="link_to"
                                                data-form="#order_form" data-control="salesperson" data-name="contact-form"
                                                data-param="id"></span></label>
                                        <select name="salesperson" id="salespersonId" class="form-control select2"
                                            style="width:100%" <?= hasPermission($order_form, 'salesperson', $operation_order) ? '' : ' disabled' ?>>
                                ...
                                <?php } ?>
                                <?php
                                if (hasPermission($order_form, 'shipping_customer_name', 'show')) { ?>
                                ...
                                <?php } ?>
                                <?php
                                if (hasPermission($order_form, 'shipping_phone', 'show')) { ?>
                                    <section class="col col-xs-5">
                            ...
                            <div class="row">
                                <div class="col col-6">
                                    <div class="row">
                                        <?php
                                        if (hasPermission($order_form, 'avalible-depots', 'show')) { ?>
                                    ...
                                    <div class="row show-container-type" style="display:none">
                                        <?php
                                        if (hasPermission($order_form, 'container-rate-avg', 'show')) { ?>
                                            <section class="col col-12">
                                    ...
                                    <div class="row show-container-type" style="display:none">
                                        <?php
                                        if (hasPermission($order_form, 'discount_code', 'show')) { ?>
                                            <section class="col col-12">
                                                <label class="input">Discount code:<span style="display:none"
                                                        class="error discount-code-error">(Invalid discount
                                                        code)</span></label>
                                ...
                                <div class="col col-6">
                                    <?php
                                    if (hasPermission($order_form, 'email_phone', 'show')) { ?>
                                    ...
                                    <?php } ?>
                                    <?php
                                    if (hasPermission($order_form, 'customer-address', 'show')) { ?>
                                        <section class="col-10">
                                    ...
                                    <?php } ?>
                                    <?php
                                    if (hasPermission($order_form, 'customer-state', 'show')) { ?>
                                        <section class=" col-10">
                                    ...
                                    <?php } ?>
                                    <?php
                                    if (hasPermission($order_form, 'customer-city', 'show')) { ?>
                                        <section class=" col-10">
                        ...
                        <?php
                        if (hasPermission($order_form, 'note', 'show')) { ?>
                            <fieldset>
                                <div class="row">
                                    <section class="col col-6">
                        ...
                        <?php } ?>
                        <?php
                        if (basename($_SERVER['PHP_SELF']) == 'order-form.php') {
                            $type_note = 'order';
                            $can_add_note = false;
                            if ($order_current_permission == true) {
                                $can_add_note = true;
...
<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDqqFUPT6qHW2hvTEfwLw6IaXs253qrlmU&v=weekly" defer>
</script>
