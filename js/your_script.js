...
var link = {
    ...
    _order_update_fields :  host +'_order_update_fields.php',
    _unpaid_inv_new_update :  host +'_unpaid_inv_new_update.php',
    _unpaid_inv_search :  host +'_unpaid_inv_search.php',
    _container_name_sku :  host +'_container_name_sku.php',
    _depot_search_city :  host +'_depot_search_city.php',
    _check_unpaid_inv_number :  host +'_check_unpaid_inv_number.php',
    _unpaid_inv_paid :  host +'_unpaid_inv_paid.php',
    _product_grp_list :  host +'_product_grp_list.php',
    _prods_list_container_type :  host +'_prods_list_container_type.php',
    _depot_all :  host +'_depot_all.php',
    _container_rate_avg :  host +'_container_rate_avg.php',
    _quote_new_update :  host +'_quote_new_update.php',
    _quote_list :  host +'_quote_list.php',
    _quote_id :  host +'_quote_id.php',
    _inventorys_depot_type :  host +'_inventorys_depot_type.php',
    _order_update :  host +'_order_update.php',
    _rate_container_grp_list :  host +'_rate_container_grp_list.php',
    _rate_container_list_container_type :  host +'_rate_container_list_container_type.php',
    _quote_new :  host +'_quote_new.php',
    _rate_container_margin_update :  host +'_rate_container_margin_update.php',
    _task_asssign_driver :  host +'_task_asssign_driver.php',
    _mail_to_customer :  host +'_mail_to_customer.php',
    /*******************************************/
    _update_int_inv :  host +'_update_int_inv.php',
    _rate_container_update_sku :  host +'_rate_container_update_sku.php',
    _inventory_search :  host +'_inventory_search.php',
    _states :  host +'_states.php',
    _states :  host +'_update_inventory_cost.php',

    /*******************************************/
    update_session :  host2 +'php/update_session.php',
    order_report :  host2 +'php/order_report.php',
    delete_file :  host2 +'php/delete_file.php',
    quote_report :  host2 +'php/quote_report.php'
...
function throw_message(mes, status, elem) {
    if (!elem) elem = '#message_form:first';
    $(elem).html('<i class="fa fa-times pointer pull-right p-r-10" onclick="$(\'' + elem + '\').hide(100)"></i>' + mes);
    $(elem).change();
    $(elem).removeClass('alert alert-danger alert-success alert-warning alert-info alert-primary');
...
var rate_mile_shorter =400;
var less_than_300ml =300;
var processing_percentage =0.029;
var OPEN = 'OPEN'
var NEEDS_TO_BE_SCHEDULED = 'NEEDS TO BE SCHEDULED'
var SCHEDULED_FOR_DELIVERY = 'SCHEDULED FOR DELIVERY'
...
var object_status ={
    'OPEN':'Open',
    'NEEDS TO BE SCHEDULED':'Scheduling',
    'SCHEDULED FOR DELIVERY':'Scheduled',
    'PICKED UP':'Out for delivery',
    'DELIVERED':'Delivered',
    'CLOSED':'Billing and close out',
    'CANCELLED':'Cancelled',
    'PROBLEMS':'Problems'
}
    ...
    $(element).select2({
        ...
        ajax: {
            ...
            data: function (params) {
                var _data = {token:_token,contact_name:params.term}
                return _data;
            },
...
/****************************/
var order_select2_el =function(element,element2,link,callback){
    $(element).select2({
        ...
        ajax: {
            ...
            processResults: function (data, params) {
                data1 = $.map(data, function (obj) {
                    var shipping_address = (obj.shipping_address !=null)?obj.shipping_address:''
                    obj.shipping_city = (obj.shipping_city !=null)?obj.shipping_city:''
                    obj.shipping_state = (obj.shipping_state !=null)?obj.shipping_state:''
                    if(obj.shipping_city !=''){
                        shipping_address =(shipping_address !='')? shipping_address+', '+obj.shipping_city:obj.shipping_city
                    }
                    if(obj.shipping_state !=''){
                        shipping_address =(shipping_address !='')? shipping_address+', '+obj.shipping_state:obj.shipping_state
                    }
                    return {
                        text:obj.text ,
                        id:obj.id,
        ...
        templateResult: function (data) {
            ...
            return '<div class="padding-5">' +
                ...
                '</div>' +
                '<div class="select2-result-repository__title">'+data.depot_name+
                '<div class="pull-right">' + data.shipping_address +'</div>' +
                '</div>' +
                '<div class="select2-result-repository__title">' + data.container_type_name +
                    '<div class="pull-right">' + email +'</div>' +
                '</div>' +
                '<div class="select2-result-repository__title">Miles: ' + data.mile +
                    '<div class="pull-right">' + data.createTime +'</div>' +
                '</div>' +
                '<div class="select2-result-repository__title">Quanity: ' + data.qty +'</div>' +
                '<div class="select2-result-repository__title"></div>' +
            '</div>';

        },
        templateSelection: function (data) {
            $(element).find('option:selected').attr('depot_id', data.depot_id);
            $(element).find('option:selected').attr('container_type_id', data.container_type_id);
            $(element).find('option:selected').attr('distance', data.mile);
            $(element).find('option:selected').attr('qty', data.qty);
            if (!data.text) return data.id;
            else return data.text;
        }
    }).change(function(){
           if(element =='#assign_order'){
               ...
               var container_type_id =$(this).find("option:selected").attr('container_type_id')
               var depot_id =$(this).find("option:selected").attr('depot_id')
               var maximun =$(this).find("option:selected").attr('qty')
               common_f.prototype.get_inventory_depot_type(container_type_id,depot_id,maximun,element2)
           }
        });
    if(callback) callback();
}
    ...
    $(element).select2({
        ...
        templateSelection: function (data) {
            ...
            if(element == '#assign_driver_id'){
                ...
                var qty = $('#task_form #assign_order').find('option:selected').attr('qty')
                var distance = $('#task_form #assign_order').find('option:selected').attr('distance')
                if(order !=''){
                    var data =  {
                        order_id:order,
                        prod_sku:product,
...
/*********************************/
var matchStart = function matchStart (term, text) {
...
/****************************/
var quote_select2_el =function(element,element2,data,callback){
    $(element).html('')
   var data1 = $.map(data, function (obj) {
    ...
    $(element).select2({
        ...
        templateResult: function (data) {
            return '<div class="padding-5">' +
                '<div class="select2-result-repository__title text-left">' + data.text +
                '<div class="pull-right">' + data.distance +' miles</div>' +
                '</div>' +
                '</div>';

        },
        templateSelection: function (data) {
             $(element).find('option').attr('distance', data.distance);
             $(element).find('option').attr('address', data.address);
            if (!data.text) return data.id;
            else return data.text;
        }
    }).change(function(){
            var depot = $(this).val();
            var mile = $(this).find(":selected").attr('distance')
            rate_avg(element2,depot,mile)
        });
...
/****************************/
var rate_avg =function(element,depot_id,distance){
    $(element).val('').trigger('change')
    $(element).html('')
    $('#order_form .show-container-type').css({"display":""})
    var link1 = link._container_rate_avg
    $(element).select2({
        ajax: {
            "async": true,
            "crossDomain": true,
            ...
            delay: 300,
            data: function (params) {
                var _data = {token:_token,text_search:params.term,depot_id:depot_id,distance:distance}
                return _data;
            },
            processResults: function (data, params) {
                data1 = $.map(data, function (obj) {
                    var avg_rate_txt = numeral(obj.avg_rate).format('$ 0,0.00')
                    return {
                        text:obj.container_type_name,
                        id:obj.container_type_id,
                        avg_rate:obj.avg_rate,
                        avg_rate_txt:avg_rate_txt,
                        avg_cost:obj.avg_cost,
                        avg_margin:obj.avg_margin,
                        distance:obj.distance,
                        rate_mile:obj.rate_mile,
        ...
        templateResult: function (data) {
            return '<div class="padding-5">' +
                '<div class="select2-result-repository__title">' + data.text +
                    '<div class="pull-right">' + data.avg_rate_txt +'</div>' +
                '</div>' +
                '</div>';

        },
        templateSelection: function (data) {
            $(element).find('option').attr('avg_margin', data.avg_margin);
            $(element).find('option').attr('avg_rate', data.avg_rate);
            $(element).find('option').attr('avg_cost', data.avg_cost);
            $(element).find('option').attr('rate_mile', data.rate_mile);
            $(element).find('option').attr('distance', data.distance);
            ...
            var container_type_id = $(this).val();
            var container_type_name = $(this).find('option:selected').text();
            var avg_margin = $(this).find("option:selected").attr("avg_margin");
            var avg_rate = $(this).find("option:selected").attr("avg_rate");
            var avg_cost = $(this).find("option:selected").attr("avg_cost");
            var rate_mile = $(this).find("option:selected").attr("rate_mile");
            var distance = $(this).find("option:selected").attr("distance");
            var container_feet_type = $(this).find("option:selected").attr("container_feet_type");
            var data ={
            ...
            container_rate_avg.prototype.create_container_avg(data,'#tbt_quote tbody')
        })
}
/****************************/
var rate_avg_data =function(element,container_data){
    $(element).val('').trigger('change')
    $(element).html('')
    $('#order_form .show-container-type').css({"display":""})
    var data1 = $.map(container_data, function (obj) {
        return {
            text:obj.container_type_name,
            id:obj.container_type_id,
            avg_rate:obj.avg_rate,
    ...
    $(element).select2({
        ...
        templateResult: function (data) {
            return '<div class="padding-5">' +
                '<div class="select2-result-repository__title">' + data.text +
                '<div class="pull-right">' + data.avg_rate +'</div>' +
                '</div>' +
                '</div>';
...
/****************************/
var salesman_select2_el =function(element,link,callback){
    $(element).select2({
        placeholder: 'Search Salesman',
        minimumInputLength: 1,
        language: {
            inputTooShort: function () {
                return 'Search Salesman';
            }
        },
        ajax: {
            "async": true,
