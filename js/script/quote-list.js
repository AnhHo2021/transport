function QuoteList() { }
QuoteList.NAME         = "QuoteList";
QuoteList.VERSION      = "1.2";
QuoteList.DESCRIPTION  = "Class QuoteList";
QuoteList.prototype.constructor = QuoteList;
QuoteList.prototype = {
    init:function(){
        QuoteList.prototype.get_quote('','','','10','','','1','#pagination_tbl-order','#tbl-order tbody');
        ...
        $('#order-list-content .btn-search').unbind('click').bind('click',function(){
            ...
            QuoteList.prototype.get_quote(from_date,to_date,status,line_num,text_search,select_type,'1',paginator_el,tbl_el);
        });
        ...
        $('#order-list-content .btn-excell').unbind('click').bind('click',function(){
            ...
            var select_type = closest.find('.select_type').val()
            var status = ''
            QuoteList.prototype.down_load_report(from_date,to_date,text_search,status,select_type,'1');
        });
    },
    /***********************************/
    get_quote:function(from_date,to_date,status,line_num,text_search,select_type,is_report,pagination_el,tbl_el){
        $('.order-content .total-record').val(0)
        $(tbl_el).html('')
        var _link =link._quote_list;
                        ...
                        $.ajax({
                            ...
                            success: function (res) {
                                ...
                                if(res.list.length > 0){
                                    var tr =  QuoteList.prototype.show_order(res.list,pagination_el)
                                    $(tbl_el).html(tr)
    ...
    show_order:function(data,el){
        ...
        data.forEach(function(item){
            var order_title_link ='<a target="_blank" href="#ajax/quote-form.php?id='+item.order_id+'">'+item.order_title+'</a>'+
                '<input type="hidden" class="order-id" value="'+ item.order_id +'">'

            var bill_to = ''
            if(item.shipping_customer_name !=null && item.shipping_customer_name !=''){
                bill_to = '<a href="./#ajax/contact-form.php?id=' + item.b_ID + '" target="_blank">' + item.shipping_customer_name + '</a>'
            }
            var div1_sale=''
            if(item.s_ID !=null && item.s_ID !='' && item.s_ID !=0){
                ...
                if((parseFloat(item.salesperson_amount) > parseFloat(item.salesperson_total_payment)) &&
                    item.order_status != CANCELLED && item.order_status != CLOSED){
            ...
            var div_container ='<div>'+
                '<div class="col col-12">'+item.container_type_name+'</div>' +
                '<div class="col col-12">Price: '+numeral(item.avg_rate).format('$ 0,0.00') +'</div>' +
                '<div class="col col-12 c-red">Cost: ('+numeral(item.avg_cost).format('$ 0,0.00') +')</div>' +
                '</div>'
            ...
            if(item.assign_task_id !=null){
                ...
                if(item.order_status != CANCELLED && item.order_status != CLOSED){
                    driver_div='<div style="margin-top:5px;">' +
            ...
            if(item.assign_task_delivery_date !=null){
                var is_current_date = new Date();
                var is_delivery_date =new Date(item.assign_task_delivery_date);
                if(is_delivery_date !='Invalid Date' && item.assign_task_delivery_date !="" ){
            ...
            tr +='<tr class="clss-row '+cls_color+' '+cls_close+'">' +
                '<input type="hidden" class="order-id" value="'+item.order_id+'">' +
                '<td>'+order_title_link+'</td>' +
                '<td>'+bill_to+'</td>' +
                '<td>'+div1_sale+'</td>' +
                '<td>'+div_container+'</td>' +
                '<td class="text-center">'+driver_div+'</td>' +
                '<td>'+total+'</td>' +
                '<td>'+createTime+'</td>' +
                '</tr>';
        });
    ...
    /**********************************/
    down_load_report:function(from_date,to_date,text_search,status,select_type,is_report){
        $.ajax({
            async: true,
            url: link.quote_report,
            type: 'post',
