...
modal_calendar.prototype = {
    init: function(){
        ...
        $('#calendar-id').on('click','.add-task',function(){
            modal_calendar.prototype.calendar_row = $(this)
            modal_calendar.prototype.number_task=1
            //modal_calendar.prototype.reset_modal_assign_task()
            Task.prototype.reset_task_modal();
            $('#calendar-task-modal').modal('show');
            var day = $(this).closest('.in-day').find('.day_text').text();
            ...
            $("#calendar-task-modal #delivery-date-task").val(current_date)
            $("#calendar-task-modal #delivery-date-task").prop("readonly",true);
        })
    ...
    view_task_in_calendar:function(data){
        ...
        if(data.length >0){
            data.forEach(function(item,index){
                ...
                item.depot_address =(item.depot_address !=null)?item.depot_address:''
                item.depot_city =(item.depot_city !=null)?item.depot_city:''
                item.depot_state =(item.depot_state !=null)?item.depot_state:''
               var depot_address = item.depot_address
               if(item.depot_city !=''){
                   depot_address =(depot_address !='')?depot_address+', '+item.depot_city:item.depot_city
               }
                if(item.depot_state !=''){
                    depot_address =(depot_address !='')?depot_address+', '+item.depot_state:item.depot_state
                }

                var shipping_address =item.shipping_address =(item.shipping_address !=null)?item.shipping_address:''

                item.shipping_city =(item.shipping_city !=null)?item.shipping_city:''
                item.shipping_state =(item.shipping_state !=null)?item.shipping_state:''
                var depot_address = item.depot_address
                if(item.shipping_city !=''){
                    shipping_address =(shipping_address !='')?shipping_address+', '+item.shipping_city:item.shipping_city
                }
                if(item.shipping_state !=''){
                    shipping_address =(shipping_address !='')?shipping_address+', '+item.shipping_state:item.shipping_state
                }

                item.shipping_customer_name =(item.shipping_customer_name !=null)?item.shipping_customer_name:''
                item.shipping_email =(item.shipping_email !=null)?item.shipping_email:''
                item.shipping_phone =(item.shipping_phone !=null)?item.shipping_phone:''

                item.driver_name =(item.driver_name !=null)?item.driver_name:''
                item.driver_phone =(item.driver_phone !=null)?item.driver_phone:''
                item.driver_email =(item.driver_email !=null)?item.driver_email:''
                var release_div =''
                if(item.release.length > 0){
                    item.release.forEach(function(item1){
                        release_div +='<div>Release: '+item1+'</div>'
                    });
                }
              ...
              tr +='<tr class="'+class1+'">' +
                  ...
                  '<td>' +
                      ...
                      '<div>'+depot_address+'</div>' +
                      '<div>'+item.depot_phone+'</div>' +
                  '</td>' +
                  '<td>' +
                  ...
                  '<td><span class="is_hide">' +
                      ...
                      '<div>'+item.container_type_name+'</div>' +
                      '<div>'+release_div+'</div>'+
                      '<div class="color-ligh-blue day-delivery">'+item.task_in_day+'</div>' +
                      '<div class="color-ligh-blue">'+item.mile+' miles</div>' +
    ...
    driver_select2_el:function(element,defined_el,modal_id,callback){
        ...
        $(element).select2({
            ...
            ajax: {
                ...
                delay: 300,
                data: function (params) {
                    var _data = {token:_token,text_search:params.term,level:level,
    ...
    update_view:function(){
        modal_calendar.prototype.number_task
        console.log("view= "+ modal_calendar.prototype.number_task)
        var closest_div = modal_calendar.prototype.calendar_row.closest('.in-day')
        var num_task = closest_div.find('.number-task').val()
        if(num_task !=''){
            num_task = parseFloat(num_task) + modal_calendar.prototype.number_task
        }else{
            num_task = modal_calendar.prototype.number_task
        }
        if(num_task >0){
            var task1 = num_task +" task";
            if(num_task >1)  task1 = num_task +" tasks";
            closest_div.find(".tooltiptext").text(task1)
            closest_div.find(".number-task").val(num_task)
            closest_div.find(".view-task").addClass('red-dart')
        }
        modal_calendar.prototype.number_task=0
        modal_calendar.prototype.calendar_row=''
    },
    /****************************/
