        ...
        $.ajax({
            ...
            success: function (data){
              ...
              var depot = '<div class="col-12 bold m-b15"><b>Strong Containers (Shipping Container Supplier, LLC)</b></div>' +
                ...
                var phone =''
                if(data.depot.shipping_phone !=null && data.depot.shipping_phone !=''){
                    phone = competitor_rate.prototype.formatPhoneNumber(data.depot.shipping_phone)
                }
                data.depot.shipping_state = (data.depot.shipping_state ==null)?'':data.depot.shipping_state
                data.depot.shipping_zip = (data.depot.shipping_zip ==null)?'':data.depot.shipping_zip
                data.depot.shipping_state = (data.depot.shipping_state ==null)?'':data.depot.shipping_state
                data.depot.shipping_state = (data.depot.shipping_state ==null)?'':data.depot.shipping_state
                data.depot.shipping_customer_name = (data.depot.shipping_customer_name ==null)?'':data.depot.shipping_customer_name
                data.depot.shipping_address = (data.depot.shipping_address ==null)?'':data.depot.shipping_address
                data.depot.email_phone = (data.depot.email_phone ==null)?'':data.depot.email_phone
                var customer = '<div class="col col-12 bold m-b15 text-right"><b>Delivery Location on '+data.driver_info.delivery_date+'</b></div>' +
                    '<div class="col col-12">'+data.depot.shipping_customer_name+'</div>' +
                    '<div class="col col-12">'+data.depot.shipping_address+'</div>' +
                    '<div class="col col-12">'+data.depot.shipping_state+'</div>' +
                    '<div class="col col-12">'+data.depot.shipping_zip+'</div>' +
                    '<div class="col col-12">'+phone+'</div>' +
                    '<div class="col col-12">'+data.depot.email_phone+'</div>'
