...
orders_list.prototype = {
            ...
            success: function (data){
                ...
                if(data.driver_paid.length >0){
                    ...
                    data.driver_paid.forEach(function(item){
                        //console.log(item);
                        sku_length = item.product_sku.length
                        var sku_l =''
                        if(item.product_sku.length >0){
                            item.product_sku.forEach(function(it){
                                sku_l +='<div class="col col-12">'+it.sku+'</div>'
                            })
    ...
    update_driver_task:function($me,task_name,task_id,
        ...
        var _link =link._task_asssign_driver;
        $.ajax({
            "async": true,
            "crossDomain": true,
            "url": _link,
            ...
            success: function (data){
                if(data.SAVE=='SUCCESS'){
                    ...
                    throw_message(data.ERROR, false,'#message_order_list');
                    return ;
                }
            }
        })
    ...
    /***********************************/
    remove_row:function(){
        orders_list.prototype.this_row.closest('tr').remove()
        orders_list.prototype.this_row =''
    },
    /***********************************/
    reset_task:function(){
