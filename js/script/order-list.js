...
OrderList.prototype = {
    init:function(){
        OrderList.prototype.get_order('','','','','','','1','#pagination_tbl-order','#tbl-order tbody');
        ...
        $('#order-list-content .btn-search').unbind('click').bind('click',function(){
            ...
            OrderList.prototype.get_order(from_date,to_date,status,line_num,text_search,select_type,'1',paginator_el,tbl_el);
        });
        ...
        $('#order-list-content .btn-excell').unbind('click').bind('click',function(){
            ...
            var select_type = closest.find('.select_type').val()
            var status = ''
            OrderList.prototype.down_load_report(from_date,to_date,text_search,status,select_type,'1');
        });
    },
    /***********************************/
    get_order:function(from_date,to_date,status,line_num,text_search,select_type,is_report,pagination_el,tbl_el){
    ...
    show_order:function(data,el){
        ...
        data.forEach(function(item){
            ...
            var div_container ='<div class="absolute-report">'+
                '<div class="col col-12">'+item.quote_temp_container_type_name+'</div>' +
                '<div class="col col-12">Quantity: '+item.qty+'</div>' +
            ...
            tr +='<tr class="clss-row '+cls_color+' '+cls_close+'">' +
                ...
                '<td>'+div1_sale+'</td>' +
                '<td>'+div_container+'</td>' +
                '<td class="'+driver_box_color+'">'+driver_div+'</td>' +
                '<td>'+total+'</td>' +
                '<td>'+payment+'</td>' +
                '<td>'+balance+'</td>' +
                '<td>'+createTime+'</td>' +
