...
find_depot.prototype = {
    ...
    /***********************************/
    find_nearest_depots:function(container_type_id,container_feet_type,zipcode,$me,find_depot,el1,el2,isedit){
         //console.log("des ="+zipcode)
        var _link =link._depot_all;
        $.ajax({
            "async": true,
            "crossDomain": true,
            "url": _link,
            ...
            success: function (data){
                ...
                if(data.depots.length >0){
                    ...
                    data.depots.forEach(function(item){
                        ...
                        depots_name.push(item["depot_name"]);
                        depots_id.push(item["depot_id"]);
                    })
                    var latitude ='';
                    var longitude='';
                           ...
                           //console.log("Latitude: " + latitude + "\nLongitude: " + longitude);
                            ...
                            service.getDistanceMatrix({
                                ...
                                if (status == google.maps.DistanceMatrixStatus.OK) {
                                    ...
                                    response.rows.forEach(function(item1){
                                        ...
                                        if(item.status =="OK"){
                                            ...
                                            if(nearest_distance.length <1){
                                                ...
                                                var add_el = {address : origin[i],distance:smallest,depot_name:depots_name[i],depots_id:depots_id[i]}
                                                ...
                                                b = b.replace( /,/g, "" );
                                                b = parseFloat(b)
                                                var add_el = {address : origin[i],distance:b,depot_name:depots_name[i],depots_id:depots_id[i]}
                                                //console.log(b)
                                    ...
                                    //console.log(nearest_distance);//return
                                    // console.log(nearest_distance);
                                    quote_select2_el(el1,el2,nearest_distance)
                                    if(isedit !='edit' && nearest_distance.length>0){
                                        var item1 = nearest_distance.slice(0,1)
                                        //console.log(item1[0].depots_id);
                                        $('#order_form #avalible-depots').find("option[value='"+item1[0].depots_id+"']").prop("selected", true).trigger('change');
                                    }
        ...
        if(depots.length >0){
            ...
            $(el + ' .select-container').unbind('click').bind('click',function(){
                var item = {
                    ...
                    available_product:$(this).find('.product_qty').val(),
                    quantity:1
                }

                $('#order_form #table_product_ordered tbody').html('')
                _orderProducts.createInputFields(item,'',item,'',1);
