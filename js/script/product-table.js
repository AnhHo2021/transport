...
ProductTable.prototype = {
    createProductRow_order: function (product_order,order_title,delivery_cost) {
       var _html =''
        product_order.forEach(function(item){
            _html +=
                '<tr>' +
                    '<input type="hidden" class="prod_id" value="' + item.prod_id + '">' +
                    '<td class="text-center">' + order_title + '</td>' +
                    '<td class="text-center">' + item.sku + '</td>' +
                    '<td class="text-center">' + item.quantity + '</td>' +
                    '<td class="text-center">' + numeral(item.container_rate).format('$ 0,0.00') + '</td>' +
                    '<td class="text-center">' + numeral(delivery_cost).format('$ 0,0.00') + '</td>' +
                    '<td class="text-center">' + numeral(item.line_total).format('$ 0,0.00') + '</td>' +
                    '</tr>';
        });
        return _html;
    },
