               ...
               if (res.startsWith('{')) {
                  ...
                  if (__invoice.ERROR == '' && __invoice.Invoice != '') {
                     ...
                     if (__invoice.ledger) {
                      ...
                      $('select[name="customer"]').append(new Option(__invoice.shipping_customer_name, __invoice.shipping_contact_id, true, true)).trigger('change');

                      if (__invoice.salesperson && __invoice.salesperson != '0') {
                          $('select[name="salesperson"]').append('<option value="' + __invoice.salesperson + '" selected>' + __invoice.sale_name + '</option>').trigger('change');
                      }

                      var ord_tbl= ProductTable.prototype.createProductRow_order(__invoice.products_ordered,__invoice.order_title,__invoice.delivery_cost)
                      $('#tb_product_show tbody').html(ord_tbl)

                      $('#tb_product_show #_total_discount').text(numeral(__invoice.discount_code_total).format('$ 0,0.00'))
                      $('#tb_product_show #_total_order').text(numeral(__invoice.total).format('$ 0,0.00'))
                      $('#tb_product_show #_payment_order').text(numeral(__invoice.payment).format('$ 0,0.00'))
                      $('#tb_product_show #_balance_order').text(numeral(__invoice.balance).format('$ 0,0.00'))
                      $('input[name="total"]').val(numeral(__invoice.total).value());
                      $('input[name="payment"]').val(numeral(__invoice.payment).value());
                      $('input[name="balance"]').val(numeral(__invoice.balance).value());

                      if (numeral(__invoice.balance).value() <= 0) {
                          $('#form_invoice #display-acct-pay').remove();
                          $('#acct-pay-modal').modal('hide');
                          $('#form-pay-acct .payment_content').prepend('<div class="alert alert-warning">The invoice had been paid. The balance is ' + numeral(__invoice.balance).format('$ 0,0.00') + '</div>');
                      } else {
                     ...
                     window.invID = __invoice.ID
                      //console.log(__invoice.payaccts)
