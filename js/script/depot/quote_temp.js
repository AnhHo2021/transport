...
quoteTemp.prototype = {
    ...
    process_show:function(quotes,el){
        ...
        var div ='<div class="col col-2" id="attach-payment">' +
            '<button class="btn btn-default form-control c-red" id="btn-payment"><b>SUBMIT</b></button>' +
            '</div>'
    ...
    calculate_sub:function(discount, discount_type){
        if($('#depot-tbl tbody tr.depot_item').length > 0){
            var sub_total = 0
            $('#depot-tbl tbody tr.depot_item').each(function(){
               var min_cost = parseFloat($(this).find('.container_rate').val())
               var qty = parseFloat($(this).find('.qty').val())
                var temp = min_cost * qty;
                var delivery_cost = $(this).find('.delivery_cost').val()
                var rate_mile = $(this).find('.rate_mile').val()
               ...
               // var price_mile = (parseFloat(rate_mile) * parseFloat(distance))
                ...
                var totol_line = parseFloat(delivery_cost) + temp;

                $('#your-cart #calculate-total').text(numeral(totol_line).format('$ 0,0.00'));

                sub_total += totol_line
            });
            if(discount_type =='$'){
    ...
    save_customer_info:function(){
        ...
        */
        ...
        var total = numeral($('#your-cart #sub-total').text()).value()
        var data_quote ={
            order_title:'',
            order_zipcode:$('#home-page #search-depot-zip').val(),
            depot_id: $('#depot-tbl tbody tr.depot_item .depot_id').val(),
            container_type_id:$('#depot-tbl tbody tr.depot_item .container_type_id').val(),
            discount_code:discount_code,
            total:total,
            balance:total,
            payment:0,
            avg_rate:$('#depot-tbl tbody tr.depot_item .container_rate').val(),
            avg_cost:$('#depot-tbl tbody tr.depot_item .avg_cost').val(),
            avg_margin:$('#depot-tbl tbody tr.depot_item .avg_margin').val(),
            delivery_cost:$('#depot-tbl tbody tr.depot_item .delivery_cost').val(),
            qty:$('#depot-tbl tbody tr.depot_item .qty').val(),
            mile:$('#depot-tbl tbody tr.depot_item .distance').val()
        }

        var customer_info ={
            shipping_customer_name:cus_name,
            shipping_address:cus_address,
            shipping_city:cus_city,
            shipping_zip:cus_zipcode,
            shipping_state:cus_state,
            shipping_phone:cus_phone,
            ...
            success: function (data){
                if(data.code !=undefined){
                    ...
                    if(data.code !=''){
                       ...
                       //alert("Your quote is ok");
                       // document.location.href =host2 +'home.php'
                    }
                }
            }
        });
