    ...
    get_quote_temp:function(code,el){
        ...
        $.ajax({
            ...
            success: function (data){
                console.log(data.more_info);
                    if(data.status =='New'){
                        quote_review.prototype.process_show(data.quotes,el)
                    }else{
                        var tr= '';
                        var total = 0;
                            var prod_info = data.more_info.products_ordered
                            var total_line = prod_info.total_line;
                            tr +='<tr>' +
                                '<td>'+prod_info.prod_name+'</td>'+
                                '<td>'+prod_info.quantity+'</td>' +
                            ...
                            '<div class="col-md-12 text-center" style="margin-top: 5px">'+
                                '<table id="quote-tbl" class="table table-responsive  table-bordered">'+
                                    ...
                                    '<tfoot>' +
                                        ...
                                        '<td colspan="2" class="p-r20" style="text-align: right"><strong>Discount</strong></td>' +
                                            '<td >'+numeral(data.more_info.discount_code_total).format('$ 0,0.00')+'</td>' +
                                        '</tr>' +
                                        '<tr style="text-align: right">' +
                                        '<td colspan="2" class="p-r20" style="text-align: right"><strong>Total</strong></td>' +
                                            '<td >'+numeral(data.more_info.total).format('$ 0,0.00')+'</td>' +
                                        '</tr>' +
                                    '</tfoot>'+
                                '</table>'+
                            '</div>'+
